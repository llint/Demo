...
{
  return "~ Real-time MMO Framework Tech Demo (0.1.0) by Lin Luo (May 2015) ~";
}
...
Model *Demo::getModel()
{
    if ( GetAutonomousEntity() )
    {
        return m_models[GetAutonomousEntity()].get();
    }
    return nullptr;
}
...
void Demo::onIdle()
{
  ...
  if(!m_bPaused)
  {
      ...
      {
          each.second->onUpdate(elapsedSeconds);
      }
  }
...
ModelPtr Demo::CreateModel()
{
    // load 'cally' model
    std::cout << "Loading 'cally' model ..." << std::endl;

    ModelPtr pModel( new Model() );
...
void Demo::OnEntityCreated(Entity::weak_ptr entity)
{
    m_models.emplace( std::move(entity), CreateModel() );
}

void Demo::OnEntityDeleted(Entity::weak_ptr entity)
{
...
void Demo::onRender()
{
  ...
  //float renderScale = m_vectorModel[m_currentModel]->getRenderScale();

  // set the projection transformation
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  gluPerspective(45.0f, (GLdouble)m_width / (GLdouble)m_height, 50.0, 5000.0);
  ...
  glLightfv(GL_LIGHT0, GL_SPECULAR, lightColorSpecular);
    ...
    if ( GetAutonomousEntity() )
    {
        Vec3 pos = GetAutonomousEntity()->GetPosition();
        float scale = m_bLeftMouseButtonDown ? 2.0f : m_bRightMouseButtonDown ? -2.0f : 0.0f;
        pos.x += scale * sinf(m_twistAngle / 180.f * M_PI);
        pos.z += scale * cosf(m_twistAngle / 180.f * M_PI);
        GetAutonomousEntity()->SetPhysics(pos, m_twistAngle);
    }
    ...
    for (auto& each : m_models)
    {
        ...
        Vec3 pos = each.first->GetPosition();
        glTranslatef(pos.x, pos.y, pos.z);
        glRotatef(m_tiltAngle, 1.0f, 0.0f, 0.0f);
        glRotatef(each.first->GetRotation(), 0.0f, 0.0f, 1.0f);

        // render model
        each.second->onRender();
